...
def listar_tabelas_silver() -> list[str]:
    if not SILVER_PATH.exists():
        return []

    tabelas = []

    for caminho in sorted(SILVER_PATH.iterdir()):
...
def carregar_silver(nome_tabela: str) -> pd.DataFrame:
    arquivo = localizar_parquet_mais_recente(SILVER_PATH / nome_tabela)

    if arquivo is None:
        raise FileNotFoundError(f"Nenhum parquet encontrado para silver.{nome_tabela}")
...
<div class="card">
    <h1>Validador da Camada Silver</h1>
    <p>Caminho Silver: <b>{{ silver_path }}</b></p>

    <form method="get">
        <label>Selecione a tabela Silver:</label><br>
        <select name="tabela">
...
<div class="card">
    <h2>silver.{{ tabela_selecionada }}</h2>
...
def index():
    ...
    if not tabelas:
        return render_template_string(
            ...
            erro="Nenhuma tabela Silver encontrada em data/silver.",
    ...
    try:
        df = carregar_silver(tabela_selecionada)
